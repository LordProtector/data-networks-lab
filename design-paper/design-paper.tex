...
\ifthenelse{\boolean{m2}}{ %text for milestone 2
  ...
  The network layer has two main tasks: build and maintain a routing / forwarding table as well as route datagrams through the network. The building and maintaining of the routing table is done in a distributed fashion using the \emph{distance vector algorithm}. Initially only the costs to the direct neighbors are known and all other costs are infinite. The minimal costs are broadcasted to the direct neighbors. If a node receives information of a lower cost path, it updates its forwarding table. If the minimal route cost changes it broadcasts this again to its neighbors. These data are send in a reliable fashion using acknowledgments. This ensures that the algorithm terminates with low cost routs for every node.
